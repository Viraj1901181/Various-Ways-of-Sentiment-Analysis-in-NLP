...
dftext1 = pd.read_csv('sentenceWS.csv')

from textblob import TextBlob

# The x in the lambda function is a row (because I set axis=1)
# Apply iterates the function accross the dataframe's rows
dftext1['polarity'] = dftext1.apply(lambda x: TextBlob(x['sentenceWS']).sentiment.polarity, axis=1)
dftext1['subjectivity'] = dftext1.apply(lambda x: TextBlob(x['sentenceWS']).sentiment.subjectivity, axis=1)
...
# -1 to -0.76 			EXTREMELY NEGATIVE
# -0.75 to -0.51 			HIGHLY NEGATIVE
# -0.50 to -0.26 			MILDLY NEGATIVE
# -0.25 to 0 			LITTLE NEGATIVE
# 0 to + 0.25 			LITTLE POSITIVE
# +0.26 to +0.50 		MILDLY POSITIVE
# +0.51 to +0.75 		HIGHLY POSITIVE
# +0.76 to +1 			EXTREMELY POSITIVE

# for i in range(len(dftext1['sentenceWS'])):
#     if dftext1['polarity'][i]>= -0.75 and dftext1['polarity']< -0.75:
#         dftext1.ix[:,'polarity':]
        
EXTREMELY_NEGATIVE = dftext1.loc[(dftext1['polarity'] >= -1) & (dftext1['polarity'] < -0.75)]      
HIGHLY_NEGATIVE    = dftext1.loc[(dftext1['polarity'] >= -0.75) & (dftext1['polarity'] < -0.50)]      
MILDLY_NEGATIVE    = dftext1.loc[(dftext1['polarity'] >= -0.50) & (dftext1['polarity'] < -0.25)]      
LITTLE_NEGATIVE    = dftext1.loc[(dftext1['polarity'] >= -0.25) & (dftext1['polarity'] < 0)]      
LITTLE_POSITIVE    = dftext1.loc[(dftext1['polarity'] >= 0) & (dftext1['polarity'] < 0.25)]      
MILDLY_POSITIVE    = dftext1.loc[(dftext1['polarity'] >= 0.25) & (dftext1['polarity'] < 0.50)]      
HIGHLY_POSITIVE    = dftext1.loc[(dftext1['polarity'] >= 0.50) & (dftext1['polarity'] < 0.75)]      
EXTREMELY_POSITIVE = dftext1.loc[(dftext1['polarity'] >= 0.75) & (dftext1['polarity'] <= 1)]      
